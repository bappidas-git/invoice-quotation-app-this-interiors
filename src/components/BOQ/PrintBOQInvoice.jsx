const PrintBOQInvoice = ({ invoice, client, organization, bankAccount }) => {
...
<html>
<head>
  <title>BOQ Invoice - ${invoice.boqInvoiceNumber}</title>
  <style>
    * { margin:0; padding:0; box-sizing:border-box; }
    body { font-family: Arial, sans-serif; color: #333; background: white; }
    .container { max-width: 900px; margin: 0 auto; padding: 20px; }
    .header { display:flex; justify-content:space-between; align-items:flex-start; padding-bottom:15px; border-bottom:3px solid #667eea; margin-bottom:20px; }
    .logo-section { flex:1; }
    .logo { max-height:60px; margin-bottom:10px; }
    .company-info { font-size:11px; color:#666; line-height:1.4; }
    .document-info { text-align:right; flex:1; }
    .document-info h1 { background:linear-gradient(135deg,#667eea 0%,#764ba2 100%); -webkit-background-clip:text; -webkit-text-fill-color:transparent; background-clip:text; font-size:26px; margin-bottom:5px; }
    .document-number { font-size:13px; color:#666; margin-bottom:3px; }
    .info-row { display:flex; gap:15px; margin-bottom:15px; }
    .info-box { flex:1; border:1px solid #e0e0e0; border-radius:6px; padding:10px; }
    .info-box-title { font-size:11px; color:#667eea; font-weight:bold; margin-bottom:6px; text-transform:uppercase; }
    ...
    .items-table th { padding:10px 8px; text-align:left; font-size:11px; font-weight:600; text-transform:uppercase; }
    .items-table td { padding:10px 8px; border-bottom:1px solid #e0e0e0; font-size:12px; vertical-align:middle; }
    .items-table tbody tr:nth-child(even) { background:#f3f0ff; }
    .text-right { text-align:right; }
    .summary-section { display:flex; justify-content:flex-end; margin-bottom:25px; }
    .summary-table { width:320px; }
    .summary-row { display:flex; justify-content:space-between; padding:6px 0; font-size:13px; }
    .summary-row.total { font-size:16px; font-weight:bold; color:#667eea; border-top:2px solid #667eea; margin-top:8px; padding-top:10px; }
    .notes-section { margin-bottom:25px; padding:12px; background:#f3f0ff; border-radius:6px; }
    .notes-title { font-size:13px; font-weight:bold; margin-bottom:6px; color:#667eea; }
    .notes-content { font-size:12px; color:#666; line-height:1.5; }
    .footer { text-align:center; padding-top:15px; border-top:1px solid #e0e0e0; color:#666; font-size:11px; }
    .footer .company-name { font-weight:bold; color:#667eea; font-size:12px; margin-bottom:3px; }
    .status-badge { display:inline-block; padding:3px 10px; border-radius:20px; font-size:11px; font-weight:bold; margin-left:8px; background:#e8f5e9; color:#2e7d32; }
...
export default PrintBOQInvoice;
